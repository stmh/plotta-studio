//! Hamburg SVG Sketch - A4 portrait with frame and imported SVG
//!
//! Controls:
...
const HAMBURG_SVG: &str = include_str!("../assets/hamburg.svg");
...
impl HamburgSketch {
    ...
    fn build_drawing(&self, ctx: &SketchContext) -> Drawing {
        ...
        if self.show_svg {
            ...
            match import_svg_string(HAMBURG_SVG) {
                Ok(imported) => {
                    ...
                    log::info!(
                        "Imported hamburg.svg: {} elements, scaled to {:.1}% (stroke widths NOT scaled)",
                ...
                Err(e) => {
                    log::error!("Failed to import hamburg.svg: {}", e);
                }
            }
        } else {
...
fn main() {
    let sketch = HamburgSketch::new();

    run_with_config(
        sketch,
        RunnerConfig::new("Hamburg, 2023")
